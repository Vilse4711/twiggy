...
def matchRightwards(md,actuals,fpos,mpos):
    ...
    if isinstance(candidate,formal):
        ...
        # Figure 6 & 8
        return matchRightwards(md,actuals,fpos,skipRight(md.modules,mpos+1)) # Skip upto a possible RB or another module
    else:
        # Failed to match
        return False,None
...
if __name__ == "__main__":
    ...
    class B(CompositeLModule):
        def __repr__(self):
            return "B()"
    class C(CompositeLModule):
        def __repr__(self):
            return "C()"
    class D(CompositeLModule):
        def __repr__(self):
            return "D()"
